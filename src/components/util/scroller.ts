...
export default class Scroller {
    ...
    if (this._isDragging) {
      ...
      if (scale != null && this.options.zooming) {
        ...
        if (oldLevel !== level) {
          ...
          // Recompute left and top coordinates based on new zoom level
          scrollLeft =
            ((currentTouchLeftRel + scrollLeft) * level) / oldLevel - currentTouchLeftRel;
  ...
  _startDeceleration(timeStamp) {
    ...
    const minVelocityToKeepDecelerating = this.options.snapping
      ? this.options.snappingVelocity
      : 0.01;
  ...
  _stepThroughDeceleration(render) {
    ...
    if (!this.options.bouncing) {
      ...
      var scrollTopFixed = Math.max(
        Math.min(this._maxDecelerationScrollTop, scrollTop),
        this._minDecelerationScrollTop,
      );
